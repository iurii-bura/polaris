...
import { FiBarChart, FiCheckCircle, FiAlertTriangle, FiXCircle, FiChevronDown, FiChevronUp, FiExternalLink } from 'react-icons/fi';
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <overallConfig.icon className={`w-4 h-4 ${overallConfig.className.split(' ').find(c => c.startsWith('text-'))}`} />
              <span className={`text-sm font-medium ${overallConfig.className.split(' ').find(c => c.startsWith('text-'))}`}>
                {overallConfig.text}
              </span>
            </div>
        ...
        <div className={`overflow-hidden transition-all duration-300 ease-in-out ${
          ...
          <div className="mt-4">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              {/* Code Coverage */}
              <div className={`tooltip tooltip-bottom flex flex-col items-center justify-center p-4 rounded-lg border ${getStatusConfig(coverageStatus).className} relative z-10`}
              ...
              {/* API Linting Issues */}
              <div className={`tooltip tooltip-bottom flex flex-col items-center justify-center p-4 rounded-lg border ${getStatusConfig(lintingStatus).className} relative z-10`}
                   data-tip="API Linting Issues">
                <FiAlertTriangle className="w-6 h-6 mb-2" />
                <div className="font-bold text-2xl">{qualityMetrics.apiLintingIssues.major + qualityMetrics.apiLintingIssues.minor}</div>
              ...
              <div className="flex items-center justify-between p-3 bg-base-200/50 rounded-lg">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <div className={`badge badge-sm ${coverageStatus === 'good' ? 'badge-success' : coverageStatus === 'fair' ? 'badge-warning' : 'badge-error'}`}>
                ...
                <div className="ml-3">
                  <div className="tooltip" data-tip="View coverage report">
                    <a
              ...
              <div className="flex items-center justify-between p-3 bg-base-200/50 rounded-lg">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <div className={`badge badge-sm ${lintingStatus === 'good' ? 'badge-success' : lintingStatus === 'fair' ? 'badge-warning' : 'badge-error'}`}>
                      {getStatusConfig(lintingStatus).text}
                    </div>
                  </div>
                  <div className="flex items-center gap-4 text-sm">
                    <span className="text-error">
                      <span className="font-medium">{qualityMetrics.apiLintingIssues.major}</span> major
                    </span>
                    <span className="text-warning">
                      <span className="font-medium">{qualityMetrics.apiLintingIssues.minor}</span> minor
                    </span>
                  </div>
                </div>
              </div>
