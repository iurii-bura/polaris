...
export function getTechnologyStatus(name: string, version: string): TechnologyStatus {
    const entry = TECHNOLOGY_WHITELIST.find((tech) => tech.name.toLowerCase() === name.toLowerCase());
    ...
    if (entry.deprecatedVersions?.some((depVersion) => version.toLowerCase().includes(depVersion.toLowerCase()))) {
        return entry.status === 'deprecated' ? 'deprecated' : 'phased-out';
    }

    // Check if version is in supported list
    if (entry.supportedVersions.some((suppVersion) => version.toLowerCase().includes(suppVersion.toLowerCase()))) {
        return entry.status;
    }
...
export function getTechnologyEntry(name: string): TechnologyWhitelistEntry | undefined {
    return TECHNOLOGY_WHITELIST.find((tech) => tech.name.toLowerCase() === name.toLowerCase());
}
