...
import { FiGitBranch, FiCheckCircle, FiAlertTriangle, FiMinus, FiChevronDown, FiChevronUp, FiExternalLink, FiUser, FiCalendar, FiFolder } from 'react-icons/fi';
...
const GitFacts: FunctionComponent<GitFactsProps> = ({ git }): ReactElement => {
  ...
  const getRepositoryStatus = (): GitStatus => {
    ...
    const lastThreeMonthsCommits = git.repositories.flatMap(repo => 
      repo.monthlyCommits.data.slice(-3)
    );
  ...
  const totalCommits = git.repositories.reduce((sum, repo) => 
    sum + repo.monthlyCommits.data.reduce((repoSum, month) => repoSum + month.commits, 0), 0
  );
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <statusConfig.icon className={`w-4 h-4 ${statusConfig.className.split(' ').find(c => c.startsWith('text-'))}`} />
              <span className={`text-sm font-medium ${statusConfig.className.split(' ').find(c => c.startsWith('text-'))}`}>
                {statusConfig.text}
              </span>
            </div>
          ...
          <div className="mt-4">
            ...
            <div className="mb-4">
              ...
              <div className="space-y-3">
                {git.repositories.map((repo) => {
                  const repoName = repo.url.split('/').pop() || 'unknown';
                  const repoCommits = repo.monthlyCommits.data.reduce((sum, month) => sum + month.commits, 0);
                  
                  return (
                    <div key={repoName} className="flex items-center gap-3 p-3 bg-base-200/50 rounded-lg">
                      <FiFolder 
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-base-content/60">{repoCommits} commits in last 12 months</div>
                      </div>
                    </div>
                  );
                })}
                  ...
                  {git.topContributors.map((contributor, index) => (
                    <div key={index} className="flex items-center gap-3 p-3 bg-base-200/50 rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-base-content/60 truncate">{contributor.email}</div>
                      </div>
                    </div>
                  ))}
                </div>
